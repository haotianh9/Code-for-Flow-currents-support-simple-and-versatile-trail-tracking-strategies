...
class CFDfunctions:
    def __init__(
        ...
        read_interval,
        level_limit,
        viz_dir="viz_cylinder2d/",
        load_velocity=True,
        load_vorticity=True,
        load_pressure=True,
        multiprocess=False,
        load_concentration=False,
        concentration_path="concentration_D=0.01/",
    ):
        ...
        self.frame_rate = round(
            1 / (ff1["BASIC_INFO"]["time"][0] - ff0["BASIC_INFO"]["time"][0])
        )
        ...
        if self.load_concentration:
            ...
            self.interval_concentration = int(
                1 / (self.timestep_concentration) / self.frame_rate
            )
            ...
            offset = 0
            for i in range(
                initframe * self.interval_concentration - offset,
                self.tn * self.interval_concentration - 1 - offset,
                self.interval_concentration,
            ):
        ...
        for n in tqdm(range(initframe, self.tn)):
            ...
            if not multiprocess:
                name = (
                    viz_dir + "/visit_dump." + index + "/processor_cluster.00000.samrai"
                )
                filename = os.path.join(source_path + name)
                ...
                num_patches = np.array(
                    f2["BASIC_INFO"]["number_patches_at_level"], dtype=np.int32
                )
                ...
                for level in range(min(MAX_LEVELS, level_limit)):
                    for patch in range(num_patches[level]):
                        ...
                        if self.load_pressure:
                            if (
                                "P"
                                in f1["processor.00000"][f"level.{level:05n}"][
                                    f"patch.{patch:05n}"
                                ]
                            ):
                                pressure = np.array(
                                    f1["processor.00000"][f"level.{level:05n}"][
                                        f"patch.{patch:05n}"
                                    ]["P"]
                                ).reshape(yn, xn)
                ...
                for i in range(multiprocess):
                    name = (
                        ...
                        + "/visit_dump."
                        + index
                        + "/processor_cluster.{}.samrai".format(str(i).zfill(5))
                    )
                    filename = os.path.join(source_path + name)
                    ...
                    for patch in range(num_patches[level]):
                        ...
                        if self.load_velocity:
                            ...
                            for i, f1 in enumerate(file_list):
                                try:
                                    U = np.array(
                                        f1[key_list[i]][f"level.{level:05n}"][
                                            f"patch.{patch:05n}"
                                        ]["U_x"]
                                    ).reshape(yn + 1, xn + 1)
                                    V = np.array(
    ...
    def adapt_time_interp(
        self,
        time=0.634,
        posX=-0.883,
        posY=0.43,
        interp_velocity=True,
    ...
    def adapt_space_interp(self, posX, posY, frame, flag):
        ...
        if flag[0] or flag[1] or flag[2]:
            ...
            for i in range(num_patch):
                ...
                if (
                    posX > self.XMIN[frame][i]
                    and posX < self.XMAX[frame][i]
                    and posY > self.YMIN[frame][i]
                    and posY < self.YMAX[frame][i]
                ):
                    # print('patch',i)
                    # print('left',XMIN[i])
                    # print('right',XMAX[i])
    ...
    def read_image(self, time, rootpath, dump_interval):
        path = (
            rootpath
            + f"/movie{int(np.floor(time*self.frame_rate/dump_interval)):04n}.png"
        )
        return path
